...
class _GameLibraryScreenState extends State<GameLibraryScreen> {
  ...
  Future<void> _load() async {
    final values = await Future.wait([
      ...
      _games.listSessions(gameType: 'card_roleplay'),
    ]);
    if (!mounted) return;
    setState(() {
...
class _GamePlayScreenState extends State<GamePlayScreen> {
  ...
  Future<void> _load() async {
    final values = await Future.wait([
      ...
      _games.listSessions(gameType: 'card_roleplay', includeArchived: true),
    ]);
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: const Color(0xFFF3F2E7),
      endDrawer: Drawer(child: _saveDrawer()),
      appBar: AppBar(
        backgroundColor: const Color(0xFFF3F2E7),
        titleSpacing: 0,
        title: Row(children: [
          ...
          Expanded(
              child: Text(session?.definitionTitle ?? '游戏',
        ...
        actions: [
          ...
          PopupMenuButton<String>(
            onSelected: (value) {
              if (value == 'end') _endGame();
            },
            itemBuilder: (_) =>
                const [PopupMenuItem(value: 'end', child: Text('结束本局'))],
          ),
        ],
      ),
      body: Column(children: [
                        ...
                        child: Column(
                            ...
                            children: [
                              ...
                              Text(message.content,
                                  style: const TextStyle(
                                      color: Color(0xFF293025), height: 1.45)),
                            ]),
                      )),
                    ]),
              );
        ...
        if (session?.status == 'ended')
          Container(
              ...
              child: const Text('本局已结束，故事摘要已经保存。', textAlign: TextAlign.center))
        else
          SafeArea(
            top: false,
            child: Padding(
              ...
              child: Row(children: [
                ...
                Expanded(
                  ...
                  decoration: InputDecoration(
                      hintText: _ooc ? '给角色和剧情的场外指令' : '你要做什么？',
                      filled: true,
