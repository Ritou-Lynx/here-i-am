...
> 执行基线：`v3-lab`
>
> 审计状态：`authority-preflight` 临时 Goal 已关闭；Goal 1 的 UI-T 真人通过；P4 R18 exact `2ed4b015` 的 Runtime 移动、持久 Undo 与两次重启真人闭环已通过，当前只验 Runtime resize；P5 / P6 与最终真人 Gate 继续阻塞
...
- P4：人工六类、R14 Runtime create / 全局卡片库、R15 快捷库连续移动 / 聊天切回 Delete、R17 精确标题正文 / 标签编辑、R18 Runtime move / persistent Undo / restart 均已真人通过。当前只验 Runtime resize；通过前不继续 remove / conflict。选中框瞬时闪烁、手动行动卡主对话噪声和 Undo 无即时成功提示仍为非阻断呈现债；
