...
9. **手机与 Desktop 是独立 App**：两端可共享统一身份与经明确约定的内容 ID，但不共用页面、信息架构或视觉壳。Desktop 不得直接嵌入、跳转或套壳手机页面；手机能力是否在 Desktop 暴露，需要独立产品决策。
...
| 工作面 | 职责 | 明确不做 |
|---|---|---|
| 首页 | 高密度展示 Card / Source / Board 的真实统计与工作入口 | 不承载自由画布，不嵌入手机生活空间，不用假数据填图表 |
| 卡片库 | 全局搜索、筛选、预览、拖入白板 | 不再拆出“资料库”“收藏”等平行库 |
| 白板 | 空间组织、分组、连线、共同工作 | 不常驻导航栏、标签栏或聊天栏 |
| 消费视图 | 阅读、观看、查看原件、生成锚点 | 不拥有白板布局，不各自发明锚点模型 |

阅读、记忆与任务中心不是当前 Desktop 一级工作面。它们的底层数据或路由可保留，但在独立 Desktop 交互方案经用户确认前，不得出现在首页或侧栏。

### 3.2 首页

首页在桌面窗口内只呈现经确认的 Desktop 工作循环。2026-08-22 真人验收确认第一阶段为六块真实模块：

- **30 天卡片活动**：用紧凑热力图表达真实创建节奏。
- **卡片构成**：分别投影卡片角色与 Source 媒介类型，不混成一个“类型”。
- **上板比例**：比较已被任意 `BoardItem` 引用与仍待整理的 Card。
- **白板增长**：以趋势图表达真实白板累计变化。
- **最近白板**：返回最近使用的画布，或进入白板索引新建、搜索。
- **待整理卡片**：显示尚未被任何白板引用的卡片，进入唯一卡片库整理。

六块模块必须来自统一 Repository / Board Store 的真实投影；空数据呈现诚实空态，不造演示数字。任何健康、财务、阅读、记忆、日程或任务模块的加入，都需要先完成独立 Desktop 交互方案并获得用户确认，不能从手机功能表自动推导。

### 3.3 卡片库

卡片库只有一个入口，默认显示“全部卡片”。筛选维度必须互相正交：

- **卡片角色**：原件卡、文字、批注、任务产物、引用；回答“这张 Card 在系统里承担什么角色”。
- **媒介类型**：文本、书籍、PDF、图片、网页、视频、音频、文件；只对具有 Source 的原件卡描述“原件是什么”。
- **标签**：来自 Card metadata 的显式用户输入；输入 `#标签` 或文字并确认后进入全局词汇，不扫描正文中的 `#片段` 自动造标签。
- 后续可继续组合归属空间、创建时间、最近使用、已在任意白板 / 未在白板等查询维度。
...
桌面和手机是两个独立 App，可在不共用 UI 的前提下对明确内容身份做跨端续接：
...
共享 `source_id / card_id / anchor_id` 不意味着 Desktop 必须拥有同名页面，也不允许 Desktop 路由直接打开手机 Widget。任何新的跨端投影都必须明确定义数据边界和 Desktop 专属交互。
