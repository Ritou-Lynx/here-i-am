...
| 工作面 | 职责 | 明确不做 |
|---|---|---|
| 首页 | 显示最近白板与待上板卡片 | 不承载自由画布，不嵌入手机生活空间 |
...
首页在桌面窗口内只呈现经确认的 Desktop 工作循环。当前仅有：

- **最近白板**：返回最近使用的画布，或进入白板索引新建、搜索。
- **待上板卡片**：显示尚未被任何白板引用的卡片，进入唯一卡片库整理。

任何生活、阅读、记忆、日程或任务模块的加入，都需要先完成独立 Desktop 交互方案并获得用户确认，不能从手机功能表自动推导。
