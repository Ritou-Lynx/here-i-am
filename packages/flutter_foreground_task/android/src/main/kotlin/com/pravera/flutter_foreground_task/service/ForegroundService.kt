...
class ForegroundService : Service() {
    ...
    /**
     * Android 15+ calls this when a foreground service hits its type-specific time budget
     * (e.g. dataSync 6h/24h, mediaProcessing). The system will shortly force-stop the service;
     * if we don't self-stop within ~10s the process is killed with
     * ForegroundServiceDidNotStopInTimeException ("Time limit already exhausted ...").
     *
     * Here I am fork: mark prefs as correctly stopped (so onDestroy will not schedule a
     * RestartReceiver alarm), drop foreground state and stopSelf gracefully. The next
     * user-foreground event, alarm, or WorkManager callback can bring the service back.
     */
    override fun onTimeout(startId: Int) {
        Log.w(TAG, "Foreground service time budget exhausted (onTimeout startId=$startId); stopping gracefully.")
        try {
            ForegroundServiceStatus.setData(this, ForegroundServiceAction.API_STOP)
        } catch (e: Exception) {
            Log.w(TAG, "Failed to update service status on timeout", e)
        }
        try {
            ...
            Log.w(TAG, "stopForeground failed on timeout", e)
        }
        stopSelf(startId)
    }
    ...
    private fun startForegroundService() {
        ...
        try {
            ...
            // not eligible to start a foreground service from background (boot, sticky restart,
            // RestartReceiver alarm, WorkManager/AlarmManager callback isolates). Android 14+
            // may also throw SecurityException if a manifest FGS type is missing or runtime
            // permission is not yet granted (microphone). Treat any failure here as
            // "service cannot come up now" and self-stop instead of crashing the process.
            Log.w(TAG, "startForeground denied; stopping service without crash.", e)
            stopSelf()
            return
        }
