...
void main() {
  group('Bilibili public/same-origin resolver', () {
    ...
    test('keeps access restriction distinct from no track', () async {
      ...
      expect((await restricted.resolve(_request)).failureKind,
          PlatformTimedTextFailureKind.accessRestricted);
      expect((await noTrack.resolve(_request)).failureKind,
          PlatformTimedTextFailureKind.noTrack);
    });

    test('classifies transport and parser failures precisely', () async {
      final network = await const BilibiliPublicTimedTextResolver(
        probe: _ThrowingProbe(),
      ).resolve(_request);
      final parser = await BilibiliPublicTimedTextResolver(
        probe: _Probe(const BilibiliSameOriginProbeResult(
          subtitleText: 'not timed text',
        )),
      ).resolve(_request);
    ...
    test('default probe is honest and keeps SRT/VTT fallback visible',
        () async {
      final result =
          await const BilibiliPublicTimedTextResolver().resolve(_request);
  ...
  test('unified resolver preserves YouTube failure classification', () async {
    ...
    final result = await resolver.resolve(const PlatformTimedTextRequest(
      providerId: 'youtube',
